...
- [x] Placement Step 1 now uses mobile-first tent count controls (`+/-`) in `Tent Manager` above the tent shelf/slot layouts (replacing the standalone add-tent form flow) for consistency with shared count-control primitives.
...
- [x] Placement save/apply actions are now unified into the shared bottom navigation flow: when current-step drafts are pending, the primary action runs save+advance (`Save & Next`), and draft-change chips render in the nav bar only when pending with singular/plural labels by count.

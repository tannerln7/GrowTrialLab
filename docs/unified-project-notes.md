...
- [x] Placement save/apply actions are now unified into the shared bottom navigation flow: when current-step drafts are pending, the primary action runs save+advance (`Save & Next`), draft-change chips render in the nav bar only when pending with singular/plural labels by count, and step blocker hints are rendered in the same bottom bar for all four steps. Step 1 next/save gating now evaluates draft shelf layout validity so unsaved but valid tent/slot drafts can proceed through `Save & Next`.

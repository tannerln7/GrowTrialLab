...
- [x] Placement Step 4 `Tent Slot Containers` now renders `Tent -> Shelf -> Slot/Tray` with horizontal shelf cells per tent so layout mirrors physical shelf groupings, and filled slots render tray cards directly while empty slots retain `Slot x` + `Empty` affordances.

...
- [x] CSS Phase 3 unification sweep completed: spacing/radius drift across experiments + shared route/component modules was normalized to a compact token ladder with viewport density scaling; shared layout primitives (`gt-page`, `gt-section`, `gt-card`, `gt-panel`, `gt-toolbar`) were added for consistent composition and easier future Tailwind utility mapping (`frontend/src/styles/tokens.css`, `frontend/src/styles/primitives.css`, `frontend/app/experiments/experiments.module.css`, `docs/ui-css-phase3-report.md`).

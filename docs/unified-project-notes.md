...
- [x] GridKit cell wrapper now supports non-button interactive shells for nested-control cards (`interactiveElement="div"` in `CellChrome`/`TrayCell`) to prevent invalid nested `<button>` markup and hydration warnings in Step 2 tray cards.
- [x] Baseline capture `Save & Next` now forces cache-fresh queue/plant refresh after save mutations (`staleTime: 0` on mutation refresh fetches), preventing stale queue replay that could jump back to already-captured plants and desync remaining-count/capture state.
...
- [x] Recipes page create form now auto-suggests and increments recipe code/name:
  - starts at `R0` / `Control` when no recipes exist
  - subsequent defaults advance to `R1` / `Treatment 1`, `R2` / `Treatment 2`, etc.

...
- [x] Placement Step 2 blocker/`Save & Next` readiness now evaluates draft tray state (draft count + draft capacities) so users can continue after staging tray additions/capacity changes without requiring persisted tray rows first.

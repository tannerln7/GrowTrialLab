...
| med | rotation rollout | Rotation logs should be tied to running lifecycle state to avoid noisy/invalid movement history during setup. | Keep rotation logging endpoint gated with lifecycle check (`running` only) and guide users to start/stop controls on Overview. |

...
| high | pre-v1 ship or post-v1 integrity hardening | Packet 3 and Packet 4 locks are UI-only; backend still accepts baseline/bin and group assignment edits when lock is set. | If audit-grade integrity is required, add backend enforcement + explicit admin unlock workflow + audit trail for lock bypasses. |

...
| low | ongoing terminology consistency | API payloads keep `bin` keys for compatibility while UI intentionally says Grade. | Keep serializers/contracts unchanged until a versioned API migration exists; enforce “Grade” in all UI copy and add a contract note for integrators to avoid mixed terms. |
| low | ongoing operator UX | Overview grouping now depends on derived tent/tray location fields; stale/null placement fields can hide physical context. | Keep overview/cockpit location payload derivation (`tent_*`, `block_*`, `tray_*`, occupancy) in shared helpers and add regression tests when touching placement serialization. |
| med | migration/legacy cleanup | Single-tray-per-block is now enforced in placement APIs, but older data snapshots may still contain blocks with multiple trays. | Add a migration/repair script before production rollout to normalize legacy block occupancy and surface any conflicts to operators. |

...
| med | feeding/assignment integrity | Feeding is now recipe-locked to `Plant.assigned_recipe`; assignment drift or missing assignments will block feed capture. | Keep groups apply as the canonical assignment writer, preserve clear “needs assignment” visibility on Overview, and avoid ad hoc assignment writes outside assignment flows. |
